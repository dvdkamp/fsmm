...
src.dir <- setwd("D:/fmc_model/fsmm-master/")

run.fsmm <-
  function(fsmm.model.input = 
    ...
    if(paste("fsmm", ext.shlib, sep = ".") %in% 
       list.files(file.path(src.dir, "code")) == FALSE){
      system('R CMD SHLIB code/fsmm.f')
    }
    
    dyn.load(paste('code/fsmm', ext.shlib, sep = "."))
    ...
    return(data.frame(model.out))
    
  }


output <- 
  run.fsmm(fsmm.model.input =
             file.path(src.dir,
                       "data/fsmm.example.forcing.vars.csv"),
           fsmm.model.pars = c(4.98,-18.80,3.01e-10,0.768,0.22),
           ## vector of model Pars, Order: A,B,K_s,M_MAX,DR_O
